...
- `packages/core` — everything, structured as a pipeline:
  ...
  2. `validate.ts` + `composition.ts` — quality (V1–V17) and security (S1–S7) rules over discovered sources; builds script inventories (sha256, network-touching detection) and composition edges.
