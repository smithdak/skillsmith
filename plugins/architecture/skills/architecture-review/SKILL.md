...
description: >-
  Judges an existing system's structure: recovers claimed invariants,
  tests them against import-graph and churn evidence, and lands ranked
  findings with options and kill triggers. Use when the user says "review
  our architecture", "is this codebase structured well", or "where is the
  coupling". Not for repo inventory (codebase-survey), new specs
  (architecture-spec), or one interface (deep-modules).
license: MIT
metadata:
