...
name: migration-plan
description: >-
  Plans a live system's transition in reversible, verified phases — data
  store, runtime, service, or API cutover. Use when the user says "plan
  the migration off Mongo", "move to the new auth service without
  downtime", "phase this upgrade", or must change a running system in
  place. Not for whether to migrate (decision-record), risk-testing the
  plan (premortem), or manual cutover scripts (wizard).
license: MIT
metadata:
