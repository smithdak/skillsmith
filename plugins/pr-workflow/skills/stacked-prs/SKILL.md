...
name: stacked-prs
description: >-
  Lands, reviews, and propagates review fixes across a stack of dependent
  GitHub PRs (A ← B ← C) with the official gh stack extension: reads
  PullRequest.stack via GraphQL as the membership authority, links missing
  members, chooses merge-forward versus sync, rebase, and push
  deliberately, fixes each review finding on the PR that introduced it and
  carries it up-stack with lease-protected pushes, re-audits threads,
  approvals, and checks after every rewrite, merges the range, verifies
  MERGED, and deletes branches only once no open PR bases on them. Use this
