...
name: webapp-testing
description: >-
  Verifies UI behavior in a local web app in a real browser via Playwright
  — clicks, assertions, screenshots, console logs. Use when the user says
  "click through the signup flow", "tell me what the console says", or
  "screenshot each step of the checkout". Not for unit-level red-green
  (tdd), building or restyling UI (frontend-craft, frontend-redesign), or
  scored design review (frontend-critique).
license: MIT
metadata:
