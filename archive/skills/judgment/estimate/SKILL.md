...
name: estimate
description: >-
  Produces calibrated estimates for uncertain quantities — duration,
  cost, size, headcount, throughput — by decomposing into estimable
  parts, anchoring each on base rates, bounding high and low
  independently, and expressing the result as a range with drivers,
  never a bare point. Use this skill when the user says "how long
  will this take", "estimate the cost", "give me a timeline",
  "size this effort", "is X feasible within Y", "how much will Z
  cost to run", or asks how sure the estimate is. Not for judging
  whether experiment results are real (growth-experiments), turning
  scope into tickets (define-work-items), looking up current prices
  or versions (ground-truth-research), or prioritizing between
  options.
license: MIT
metadata:
