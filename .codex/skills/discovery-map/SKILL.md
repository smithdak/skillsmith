---
name: "discovery-map"
description: "Plans work too large and too foggy for one session as a shared map of investigation tickets, resolved one per session until the route to a named destination is clear. Use this skill when the user wants to \"chart a discovery map\", \"break the unknowns into tickets\", plan a large vague effort across sessions, or \"work through the next ticket on the map\". Not for orienting in existing code, writing a single work item, or scheduling known work."
license: "MIT"
metadata:
