...
description: "Adds motion to a tldraw canvas — animating shape position, rotation, and props over time with easing, animating the camera through a canvas, and authoring durable requestAnimationFrame loops embedded in a tldraw-offline document so the canvas keeps animating whenever it is opened. Use this skill when the user says \"animate this tldraw diagram\", \"make these shapes move\", \"add an animation to my tldraw canvas\", \"create a tldraw animation loop\", or \"animate the camera through my board\". Not for generating a static .tldr (see tldraw-diagram), exporting a canvas to an image (see tldraw-export), or CSS / web / video-file animation outside tldraw."
license: "MIT"
metadata:
