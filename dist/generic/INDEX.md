...
| `frontend-craft` | engineering | Designs and builds high-craft web UI that does not read as machine-generated: infers the brief, sets explicit variance/motion/density dials, and forbids the generic defaults that mark AI-built pages. |
| `frontend-redesign` | engineering | Upgrades the craft of an existing web UI without a rewrite: scans the current interface, diagnoses specific defects against an audit checklist, and applies the smallest changes that raise quality. |
