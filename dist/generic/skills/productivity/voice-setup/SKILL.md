...
name: voice-setup
description: >-
  Interactive guided setup that persists the output-contract reply rules into
  real instruction files: detects which harness surfaces exist (AGENTS.md,
  CLAUDE.md, opencode configs, global scopes), interviews for scope and rule
  toggles, renders a marker-delimited managed block via scripts, and verifies
  the result idempotently. Use when the user says "set up the output
  contract", "/voice-setup", "make replies concise in this repo", "configure
  the agent output style", "persist these conciseness rules", or wants terse
  output applied project-wide or machine-wide. Not for refactoring code to be
  shorter, editing README prose, explaining harness config formats for their
  own sake, or one-off rewording that is never persisted.
license: MIT
metadata:
