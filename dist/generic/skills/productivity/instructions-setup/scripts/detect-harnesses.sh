#!/bin/sh
# detect-harnesses.sh — inventory the agent-instruction surfaces the
# communication contract can attach to. Read-only; prints KEY=VAL lines;
# exits 0 even when nothing is found so the setup interview can proceed on
# partial facts. A file counts as managed when it holds any block whose
# marker starts with the agent-voice:output-contract prefix (v1 or v2).
set -u
