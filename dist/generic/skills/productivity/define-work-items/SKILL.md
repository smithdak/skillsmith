...
description: >-
  Writes work items to a business-analyst standard, with testable acceptance
  criteria, so complete an implementer needs to ask nothing. Use when the user
  says "write a ticket", "create a work item", "turn this into user stories",
  or wants a bug or feature written up for someone else to build. Not for a
  build-ready PRD (feature-spec), in-flight state (handoff), prioritization,
  or estimation.
license: MIT
metadata:
